...
# 2. Add explicit WSGI aliases for Vercel
application = app
handler = app
...
def build_m3u(data):
    channels = data if isinstance(data, list) else data.get("channels", data.get("result", [data]))
    m3u_lines = ["#EXTM3U\n"]

    for ch in channels:
        name = ch.get("name") or ch.get("title") or "Unknown Channel"
        channel_id = ch.get("channel_id") or ch.get("id") or ch.get("tvg_id") or ""
        category = ch.get("category") or ch.get("group") or ch.get("group_title") or ""
        logo = ch.get("logo") or ch.get("tvg_logo") or ch.get("icon") or ""

        drm_key = ch.get("drm") or ch.get("license_key") or ch.get("key") or ch.get("clearkey") or ""
        user_agent = ch.get("userAgent") or ch.get("user_agent") or ch.get("agent") or ""
        token = ch.get("token") or ch.get("cookie") or ""
        mpd_link = ch.get("mpd") or ch.get("link") or ch.get("url") or ch.get("mpd_url") or ""

        m3u_lines.append(f'#EXTINF:-1 tvg-id="{channel_id}" group-title="{category}" tvg-logo="{logo}",{name}')
